...
if [ ! -s "$wikimatrix_path/WikiMatrix.$1-$2.tsv" ] ; then
  ...
  cd "$wikimatrix_path"
  wget --no-clobber "https://dl.fbaipublicfiles.com/laser/WikiMatrix/v1/WikiMatrix.$1-$2.tsv.gz"
  cd "$data_dir"
  echo '> done'
fi
...
wikimatrix_file=$wikimatrix_path/WikiMatrix.$1-$2.tsv
if [ ! -s "${wikimatrix_file}.filtered" ]; then
...
if [ ! -s "${wikimatrix_file}_sample_${sample}" ]; then
...
if [ ! -s "${wikimatrix_file}.moses.$1" ]; then
  cd "$wikimatrix_path"
...
if [ ! -d "${nltk_corpora_dir}" ]; then
  ...
  cd "$nltk_corpora_dir"
  wget --no-clobber http://www.nltk.org/nltk_data/packages/corpora/stopwords.zip
  unzip stopwords.zip
  cd "$root_dir"
  echo '> done'
fi
...
if [ ! -d "${nltk_punkt_dir}" ]; then
  ...
  cd "$nltk_punkt_dir"
  wget --no-clobber http://www.nltk.org/nltk_data/packages/tokenizers/punkt.zip
  unzip punkt.zip
  cd "$root_dir"
  echo '> done'
fi
...
if [ ! -d "${nltk_tag_dir}" ]; then
  ...
  cd "$nltk_tag_dir"
  wget --no-clobber http://www.nltk.org/nltk_data/packages/taggers/averaged_perceptron_tagger.zip
...
if [ ! -d "${nltk_corpora_dir}/wordnet" ]; then
  ...
  mkdir -p "$nltk_corpora_dir"
  cd "$nltk_corpora_dir"
  wget --no-clobber http://www.nltk.org/nltk_data/packages/corpora/wordnet.zip
  unzip wordnet.zip
  cd "$root_dir"
  echo '> done'
fi
