...
from transformers import (
    semdiv_convert_examples_to_features as convert_examples_to_features,
)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type != "distilbert":
                ...
                inputs["token_type_ids_b"] = (
                    batch[6] if args.model_type in ["bert", "xlnet"] else None
                )  # XLM, DistilBERT and RoBERTa don't use segment_ids
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and not args.tpu:
                ...
                if (
                    ...
                    if (
                        args.local_rank == -1 and args.evaluate_on_training
                    ):  # Only evaluate when single GPU otherwise metrics may not average well
                        results_training = evaluate(
                            args,
                            model,
                            tokenizer,
                            global_step,
                    ...
                    tb_writer.add_scalars(
                        "f1",
                        {
                            "training_f1": results_training["f1"],
                            "eval_f1": results_eval["f1"],
                        },
                        global_step,
                    )
                    tb_writer.add_scalars(
                        "loss",
                        {
                            "training_loss": results_training["total_loss"],
                            "eval_loss": results_eval["total_loss"],
                        },
                        global_step,
                    )
...
def evaluate(
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        if evaluate_on_training:
            ...
            eval_dataset = load_and_cache_examples(
                args, eval_task, tokenizer, evaluate=True, evaluate_on_test=True
            )
            ...
            if preds is None:
                ...
                if not evaluate_on_test:
                    ...
                    out_label_ids = np.append(
                        out_label_ids, inputs["labels_b"].detach().cpu().numpy(), axis=0
                    )
            else:
                preds = np.append(preds, logits_a.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(
                    out_label_ids, inputs["labels_a"].detach().cpu().numpy(), axis=0
                )
                if not evaluate_on_test:
                    preds = np.append(preds, logits_b.detach().cpu().numpy(), axis=0)
                    out_label_ids = np.append(
...
def load_and_cache_examples(
    ...
    if not evaluate_on_test:
        all_input_ids_a = torch.tensor(
            [f.input_ids_a for f in features], dtype=torch.long
        )
        all_attention_mask_a = torch.tensor(
            [f.attention_mask_a for f in features], dtype=torch.long
        )
        all_token_type_ids_a = torch.tensor(
            [f.token_type_ids_a for f in features], dtype=torch.long
        )

        all_input_ids_b = torch.tensor(
            [f.input_ids_b for f in features], dtype=torch.long
        )
        all_attention_mask_b = torch.tensor(
            [f.attention_mask_b for f in features], dtype=torch.long
        )
        all_token_type_ids_b = torch.tensor(
            [f.token_type_ids_b for f in features], dtype=torch.long
        )
    ...
    if not evaluate_on_test:
        dataset = TensorDataset(
            ...
            all_labels_a,
            all_input_ids_b,
            all_attention_mask_b,
            all_token_type_ids_b,
            all_labels_b,
        )
    else:
        dataset = TensorDataset(
            all_input_ids_a, all_attention_mask_a, all_token_type_ids_a, all_labels_a
        )
    return dataset
