    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if mode != "dev":
            ...
            if preds_sent is None:
                ...
                preds_sent = np.append(
                    preds_sent, logits_second.detach().cpu().numpy(), axis=0
                )
                sent_label_ids = np.append(
                    sent_label_ids, inputs["label"].detach().cpu().numpy(), axis=0
                )
            ...
            if preds_tok is None:
                ...
                preds_tok = np.append(
                    preds_tok, logits_sec_seq.detach().cpu().numpy(), axis=0
                )
                tok_label_ids = np.append(
                    tok_label_ids, inputs["label_ids_dv"].detach().cpu().numpy(), axis=0
                )
...
def load_and_cache_examples(
    ...
    if mode == "train" or mode == "dev":
        all_input_ids_eq = torch.tensor(
            [f.input_ids_eq for f in features], dtype=torch.long
        )
        all_input_mask_eq = torch.tensor(
            [f.input_mask_eq for f in features], dtype=torch.long
        )
        all_segment_ids_eq = torch.tensor(
            [f.segment_ids_eq for f in features], dtype=torch.long
        )

    all_sentence_lbl = torch.tensor([int(f.label) for f in features], dtype=torch.long)
    all_input_ids_dv = torch.tensor(
        [f.input_ids_dv for f in features], dtype=torch.long
    )
    all_input_mask_dv = torch.tensor(
        [f.input_mask_dv for f in features], dtype=torch.long
    )
    all_segment_ids_dv = torch.tensor(
        [f.segment_ids_dv for f in features], dtype=torch.long
    )
...
def subword2token_labels(
            ...
            for id_, line in enumerate(f):
                ...
                if mode != "dev":
                    ...
                    total_gold_tgt.extend(
                        [binarize_map[x] for x in line[6].rstrip().split(" ")]
                    )
                    ...
                    total_gold_tgt.extend(
                        [binarize_map[x] for x in line[5].rstrip().split(" ")]
                    )
    ...
    f1 = f1_score(
        total_gold_src + total_gold_tgt,
        total_predictions_src + total_predictions_tgt,
        average=None,
    )
...
def get_args():
    ...
    ## Required parameters
    parser.add_argument(
        "--node", default=None, type=str, required=True, help="Node used for training."
    )
    parser.add_argument(
        "--data_dir", default=None, type=str, required=True, help="The input data dir"
    )
    parser.add_argument(
        "--synth_data_dir",
        default=None,
    ...
    ## Other parameters
    parser.add_argument(
        "--labels", default="", type=str, help="Path to a file containing all labels"
    )
    parser.add_argument(
        "--config_name",
        default="",
    ...
    parser.add_argument(
        "--evaluation_set", type=str, help="Evaluation set", default="dev"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--margin", type=float, default=5, help="margin for triplet loss"
    )
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.best_checkpoint:
            ...
            for checkpoint in checkpoints:
                ...
                if args.evaluation_set != "dev":
                    output_predictions_file = os.path.join(
                        str(checkpoint) + "_" + args.evaluation_set + "_predictions.txt"
                    )
                    _, preds_sent, sigm, tok_preds_list = evaluate(
                        args,
                        model,
