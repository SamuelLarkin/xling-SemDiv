...
def main():
    ...
    with open(output_corpus, "w") as output, open(o.src_sents, "r") as src, open(
        o.tgt_sents, "r"
    ) as tgt:
        src = src.readlines()
        tgt = tgt.readlines()
        ...
        for similarity_index in sim_sorted:
            ...
            if (
                levenshtein_distance(src_sentence, tgt_sentence) < 30
                or perc_numeric(src_sentence) > 0.3
            ):
                continue

            output.write(
                "{0}\t{1}".format(src[similarity_index].strip(), tgt[similarity_index])
            )
