...
def main():
    ...
    parser.add_argument(
        "--similarity",
        help="flag that tests whether a similarity score is provided for bitexts",
        default=True,
    )
    parser.add_argument(
        "--threshold",
        help="similarity threshold -- assumes ordered corpus",
        default=None,
    )
                    ...
                    if levenshtein(src_sent, tgt_sent) < 20:
                        ...
                        if perc_numeric(en) < 0.15:
                            ...
                            if args.src == "en":
                                output_bitext.write(
                                    "{0}\t{1}\n".format(
                                        " ".join(src_sent), " ".join(tgt_sent)
                                    )
                                )
                            else:
                                output_bitext.write(
                                    "{0}\t{1}\n".format(
                                        " ".join(tgt_sent), " ".join(src_sent)
                                    )
                                )
                        else:
                            continue
